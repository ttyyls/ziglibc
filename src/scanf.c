    ...
    *fmt += 1;
    ...
    if (c == 's') {
      *fmt += 1;
      if (mod != MOD_NONE) {
        fprintf(stderr,
                "scanf modifier for specifier 's' is not implemented\n");
        abort();
      }
      ...
      *fmt += 1;
      ...
      fprintf(
          stderr,
          "scanf modifier or specifier '%c' is invalid or not implemented\n",
          c);
      abort();
    }
  ...
  while (1) {
    ...
    case SCAN_KIND_STRING: {
      ...
      while (c != 0) {
        s_arg[total_read] = c;
        total_read++;
        if (scan.string.width != -1 && total_read >= scan.string.width)
          break;
        c = reader->read(reader);
        if (isspace(c))
          break;
    ...
    case SCAN_KIND_HEX: {
      ...
      switch (store.mod) {
      case MOD_NONE:
        *va_arg(args, int *) = store.i;
        break;
      case MOD_LONG:
        *va_arg(args, long int *) = store.li;
        break;
      default:
        assert(0);
      }
